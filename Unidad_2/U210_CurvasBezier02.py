""" Dibujando curvas: Curvas de Bézier
    ...
    EN ESTA VERSION SE USA SOLO PYTHON, SIN NUMPY, ESTO ES, EL USO DE LISTAS
    """
...
def dibujar_curva_bezier_manual():
    ...
    for i in range(101):
        ...
        punto_x = (1-t)**3 * puntos_de_control[0][0] + 3*(1-t)**2*t * puntos_de_control[1][0] + 3*(1-t)*t**2 * puntos_de_control[2][0] + t**3 * puntos_de_control[3][0]
        punto_y = (1-t)**3 * puntos_de_control[0][1] + 3*(1-t)**2*t * puntos_de_control[1][1] + 3*(1-t)*t**2 * puntos_de_control[2][1] + t**3 * puntos_de_control[3][1]
