""" El siguiente código permite hacer uso del concepto de perspectiva en una escena 3D. 
    Usando OpenGL y GLFW, se dibujan tres cubos de alambre a distintas distancias en el eje Z.
    Se utiliza gluPerspective para definir una proyección en perspectiva y gluLookAt para posicionar la cámara.
    La cámara puede ser movida hacia adelante y hacia atrás con las teclas ARRIBA y ABAJO,
    y el campo de visión (FOV) puede ser ajustado con las teclas IZQUIERDA y DERECHA.
    ...
    flecha izquierda    Haces zoom in (FOV más pequeño)
    flecha derecha      Haces zoom out (FOV más grande)

    Conclusión visual:
    ------------------
    gluLookAt() define dónde está la cámara y hacia dónde mira.
    gluPerspective() controla cómo la cámara proyecta el mundo, es decir, el tipo de lente usara.
