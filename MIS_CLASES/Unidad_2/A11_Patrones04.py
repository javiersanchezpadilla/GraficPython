""" En este ejercicio controlaremos la velocidad de los patrones
    ...
    Solo cambia la forma en que crea la lista de los objetos MapaBitsDelPackman

    Este es un ejemplo de cómo usar MOLDEs (clases) para crear múltiples objetos

    Usas el MOLDE 'Personaje' para crear el objeto 'jugador_guerrero'
        jugador_guerrero = Personaje(nombre="Kael", vida=100, ataque=15)    

    Vuelves a usar el mismo MOLDE 'Personaje' para crear el objeto 'enemigo_orco'
        enemigo_orco = Personaje(nombre="Grum", vida=80, ataque=10)

    Creamos la lista vacía para guardar a todos
        lista_de_personajes = []

    Añadimos los objetos creados a la lista
        lista_de_personajes.append(jugador_guerrero)
        lista_de_personajes.append(enemigo_orco)

    Si quieres crear un tercer personaje y añadirlo directamente:
        lista_de_personajes.append(Personaje(nombre="Mago", vida=70, ataque=20))
...
if __name__ == "__main__":
    ...
    # Crear múltiples objetos MapaBitsDelPackman con diferentes velocidades y patrones
    lista_packmans = []

    lista_packmans.append(MapaBitsDelPackman(1, 10, 1, 1, 5, que_packman))
    lista_packmans.append(MapaBitsDelPackman(1, 110, 3, 5, 1, que_packman))
    lista_packmans.append(MapaBitsDelPackman(1, 210, 6, 10, 10, que_packman))
    lista_packmans.append(MapaBitsDelPackman(1, 310, 9, 2, 20, que_packman))
    lista_packmans.append(MapaBitsDelPackman(1, 410, 12, 4, 7, que_packman))
